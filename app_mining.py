...
DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "storage", "lyon_full_history.csv")

st.set_page_config(page_title="Lyon ML Miner", page_icon=None, layout="wide")

# --- Custom Styling ---
# Lapis Blue is approximately #26619C
st.markdown("""
    <style>
...
def load_and_mine():
    ...
    df['time'] = pd.to_datetime(df['time']) # On convertit en format date
...
with tab1:
    ...
    st.caption("1 = Janvier, 12 = Décembre. L'hiver est généralement plus pollué.")
    ...
    st.pyplot(fig)
    st.caption("On observe graphiquement que les AQI les plus élevés surviennent principalement lorsque le vent est faible.")
